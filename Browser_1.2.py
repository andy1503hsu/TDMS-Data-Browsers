...
class MainWindow(QMainWindow):
    ...
    def clicked(self, groupText, channel):
        ...
        # print("Before insert/delete:", self.interLayout.count())
        ...
        else: # have to figure out the weird interaction overlay thing...
        ...
        # print("After insert/delete:", self.interLayout.count())

    def initGraphWidget(self):
        self.j = -1
        self.graphWidget.setTitle(color="b", size="30pt")
        pg.setConfigOption('foreground', 'w')
        self.legend = self.graphWidget.addLegend(offset = (100,100), labelTextSize ='12pt') # WHY IS THE TEXXT SIZE NOT CHANGING
        self.graphWidget.showGrid(x=True, y=True)
        styles = {"color": "#4c55ff", "font-size": "20px"}
        self.graphWidget.setLabel("bottom", "Time", **styles)
        styles = {"color": "#66f0ff", "font-size": "20px"}
        self.graphWidget.setLabel("left", "Non-Digital Channels", **styles)
    ...
    def setInteraction(self, group, channel, plotItem, i):
        # for group_channel, plotItem in enumerate(self.selectedButtons.items()):
            #print(group_channel)
...
def test():
    num = int(input("Enter 0 for Venturi, 1 for GNC, 2 for ???: "))
    file_open = ""
    if num == 0:
        file_open = "DataRPVenturiData.tdms"
    if num == 1:
        file_open = "GN2Test Data.tdms"
    if num == 2:
        file_open = "Data1Data File2020.tdms"
    tdms = TDMS_File(file_open)
    # tdms.debug()
    run_app(tdms)
